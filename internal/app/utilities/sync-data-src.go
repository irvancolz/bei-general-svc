...
func GetLatestPJSPPACompanies(c *gin.Context) ([]model.ContactPersonSyncCompaniesResource, error) {
	err_host := godotenv.Load(".env")
	if err_host != nil {
		fmt.Println(err_host)
	}
	host := os.Getenv("SERVICE_PJSPPA_HOST")

	url := host + "/get-pjsppa-to-sync-with-contact-person"
	tokens, _ := c.Get("token")
